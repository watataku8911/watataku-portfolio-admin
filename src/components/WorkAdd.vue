    ...
    <table border="1" class="table" v-show="isLoadingFlg">
        <tbody>
            <tr>
                ...
                <td><input type="text" id="addWorkWname" name="addWorkWname" value="" size="100" v-model="title"></td>
            </tr>
            <tr>
                <th>URL&nbsp;<span class="required">必須</span></th>
                <td><input type="text" id="addWorkUrl" name="addWorkUrl" value="" size="100" v-model="url"></td>
            </tr>
            <tr>
                <th>コメント&nbsp;<span class="required">必須</span></th>
                <td><textarea id="addWorkBody" name="addWorkBody" cols="100" rows="15" v-model="description">{}</textarea></td>
            </tr>
            <tr>
                <th>画像&nbsp;<span class="required">必須</span></th>
                <td>
                    <input type="file" id="addWorkImage" name="addWorkImage" mulitple="multiple" @change="onDrop">
                    <p v-if="this.imagePath">
                        <img :src="this.imagePath" width="200" height="200" :alt="this.imagePath" class="editImage" />
                    </p>
            ...
            <tr>
                ...
                <td>
                    <select name="categories" v-model="categoryId">
                        <option v-for="item in this.categories" v-bind:key="item.cetegory_id" :value="item.category_id">
                            {{item.category_name}}
                        </option>
                    </select>
                </td>
            ...
            <tr>
                <td colspan="2" class="submit" v-if="id == 0" v-show="isLoadingFlg"><input type="submit" value="登録" @click="add()"></td>
                <td colspan="2" class="submit" v-if="id != 0" v-show="isLoadingFlg"><input type="submit" value="編集" @click="edit()"></td>
            </tr>
        </tbody>
    </table>
    </div>
...
export default {
    ...
    created() {
      db.collection("categories").orderBy("category_id").get().then((query) => {
        const categoryList = [];
        query.forEach((doc) => {
          const data = doc.data();
    ...
    mounted() {
        ...
        this.id = id;
        if(id != 0) {
            db.collection("works").doc(id).get().then((query) => {
    ...
    methods: {
        ...
        edit() {
            ...
            if(!errFlg) {
                db.collection("works").doc(this.id).update({
                    title: this.title,
                    url: this.url,
                    description: this.description,
                    imagePath: this.imagePath,
